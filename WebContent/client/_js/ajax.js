...
function setXMLHttpRequest() { //XMLHttpRequest객체생성함수
	if (window.ActiveXObject) {//현재 브라우저가 ActiveXObject를 지원하다면 (IE라면)
		try {
		...
		//현재 브라우저가 XMLHttpRequest를 지원한다면(크롬, 사파리, 오페라,IE버전 11이상)
 ...
 * 		0(아무일 없음), 1(open호출) 2(send호출) 3(데이터 받는 중) 4(데이터를 다 받음)
 * xhr.status :서버의 상태를 숫자코드로 표현
 *      200(OK) 404(페이지없음) 500(서버실행 중 에러)
 * xhr.onreadystatechange : readyState의 정수값이 변할 때를 표현하는 이벤트 속성
 * 		사용예) xhr.onreadystatechange= callback함수명;
